...
def reset_ball():
    ...
    ball_speed[1] = direction * base_speed * math.sin(angle)
#Game loop
running = True
while running:
    ...
    if ball.colliderect(l_paddle) or \
        ...
        ball_tweak = random.uniform(-2,2)
        ball_speed[1] += ball_tweak
    ...
    pygame.draw.rect(my_screen,(0,255,0), l_paddle)
    pygame.draw.rect(my_screen, (0, 255, 0), r_paddle)
    pygame.draw.ellipse(my_screen, (255,0,0) , ball)
    score_text = font.render(f"{score[0]} - {score[1]}",True,
                                 (255,255,255))
    my_screen.blit(score_text, (380,10))
    pygame.display.flip()
    clock.tick(60)
pygame.quit()